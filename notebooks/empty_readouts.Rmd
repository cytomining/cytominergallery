...
empty_frequency %<>%
  ...
  mutate(t_scale =
           as.integer(
             str_match(
               cp_feature_name,
               "Texture_([A-Za-z0-9]+)_([A-Za-z]+)_([0-9]+)_([0-9]+)"
             )[[4]]
           )) %>%
  mutate(t_angle =
           as.integer(
...
empty_frequency %<>%
  ...
  mutate(t_feature_name =
          str_match(
               feature_name,
               "([A-Za-z0-9]+)_([A-Za-z]+)_([0-9]+)_([0-9]+)"
             )[[2]]
           ) %>%
  ungroup()
```
...
Exclude 
- `EulerNumber` - *this is not always be uninformative, but we drop in this analysis*
...
- `Nuclei_Neighbors_*_2` is mostly `0` because two pixels is too little - *this is not always be uninformative, but we drop in this analysis* 
...
Repeat the previous query but count `NA` instead.
...
Also check if Granularities have any `NA`
