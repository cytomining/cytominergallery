...
#sampled_objects %>% select(matches("^Cells_|^Cytoplasm_|^Nuclei_")) %>% parallel.cor(., n.splits = n.splits, n.cores = n.cores, cov.fun = "custom_multi_covar")
...
source("notebooks/custom_covar.R")

n.splits <- 2
n.cores <- 4
n.top.cor <- 5          # how many top correlated feature
                        # to each given feature is shown

sampled_objects %>%
  select(matches("^Cells_|^Cytoplasm_|^Nuclei_")) %>% 
  parallel.cor(., n.splits = n.splits, n.cores = n.cores, cov.fun = "custom_multi_covar") %>% 
...
rbenchmark::benchmark(sampled_objects %>%
  select(matches("^Cells_|^Cytoplasm_|^Nuclei_")) %>%
  parallel.cor(., n.splits = n.splits, n.cores = n.cores, cov.fun = "custom_multi_covar"), replications = 3) %>% print

rbenchmark::benchmark(sampled_objects %>%
  select(matches("^Cells_|^Cytoplasm_|^Nuclei_")) %>%
  cor, replications = 3) %>% print
